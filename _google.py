...
def search(url):
    ...
    if sys.version_info[0] > 2:
        with open('{}/source.html'.format(os.getcwd()), 'w+', encoding='utf-8', errors='replace') as f:
            f.write(source)
    else:
        with io.open('{}/source.html'.format(os.getcwd()), 'w+', encoding='utf-8') as f:
            f.write(source)
...
def google(url, metadata, query, limit):
    ...
    links = [json.loads(i.text)["ou"]
             for i in soup.find_all("div", {"class":"rg_i Q4LuWd tx8vtf"})]
    ...
    for a in soup.find_all("div",{"class":"rg_i Q4LuWd tx8vtf"}):
